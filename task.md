...
## 2026-08-09 — 보스 픽셀 디스플레이 4종 수정 (짤림 · 조명 · 가림 · 겹침 아웃라인)

**1. 보스가 네모나게 짤리던 버그 (근본 원인 확정)** — `BossPixelCamera`의 `orthographicSize`가
9로, `BossPixelDisplay` Quad 스케일이 32x18로 상수 고정돼 있었다. 그런데 실제 화면은
`SectionCamera.EnterRoom`이 룸 크기대로 ortho를 바꾼다 — 씬의 BossRoomTrigger는 48x27 / 40x22.5 /
30x16.875(전부 useLetterbox=1)라 ortho가 13.5 / 11.25 / 8.44가 된다. 48x27 룸에서 32x18 Quad는
화면의 가로세로 66.7%만 차지 → 화면 좌표 0.167~0.833 구간에 하드 엣지가 생긴다(사용자 스크린샷의
잘린 사각형 위치와 일치). 동시에 픽셀 카메라도 32x18 월드 영역만 찍으니 스케일 30인 보스 몸통이
RT 경계에서 실제로 잘려 나갔다. → `BossPixelResolutionController`가 매 프레임(그리고
`RenderPipelineManager.beginCameraRendering`에서 한 번 더) 메인 카메라의 ortho/aspect를 그대로
따라가고 Quad도 정확히 그 화면 크기로 맞춘다(RT ↔ 화면 1:1). 픽셀 카메라 depth도 메인보다
1 낮춰 같은 프레임 그림이 쓰이게 했다(전엔 depth 0 > -1이라 한 프레임 늦은 그림이 표시됨).

**2. 보스 빛이 타일·모든 요소를 비추게** — 씬에 Global Light 2D가 하나도 없고 보스의 빔(Point,
outer 45°, range 25)만 있어서 부채꼴이 지나가는 자리 말고는 전부 검게 남았다. `BossEyeTracker`에
눈 위치를 따라다니는 넓은 원형 광원(`BossEyeGlowLight`, 각도 360, 기본 반경 45 = 가장 큰 룸
48x27을 덮음, volumetric off)을 추가. 두 라이트 모두 `targetSortingLayers`를 현재 전체 소팅
레이어로 명시(URP 17.3 `Light2D.Awake`가 기본값을 채워주지만 레이어 추가 시 깨지지 않도록).
출처: `Light2DCullResult.cs:131` — 라이트는 `camera.cullingMask & (1 << light.gameObject.layer)`로
컬링된다(그래서 빛은 PixelBoss가 아니라 Default 레이어에 있어야 한다).

**3. 보스가 일부 오브젝트를 감추던 버그** — Quad의 SortingOrder가 1이라 광원 오브젝트
(LightObjects/OBJ_*, order 0)를 덮었다. 컨트롤러가 런타임에 order를 강제한다. 겸사겸사
`renderer.material` 복제(에디트 모드에서 호출될 때마다 머티리얼이 씬에 눌러앉아 "(Instance)"가
36번 중첩돼 있었다)를 `MaterialPropertyBlock`으로 교체.

⚠️ **값 결정 과정(플레이 모드 실측)**: 처음엔 -100으로 내렸더니 보스가 화면에서 통째로 사라졌다.
배경 하늘 타일맵 `Grid (4)/d`가 order **-1**로 화면을 100% 덮고 있어 그 뒤로 숨은 것. 즉 보스가
들어갈 자리는 "하늘(-1)보다 앞 + 나머지 전부보다 뒤"뿐이다. 씬에서 order -1인 지형 스프라이트
(Floor·Plat·Wall)는 **전부 비활성**(옛 테스트 잔재)이라 실질적으로 -1은 하늘 하나 → **order 0**이면
하늘을 이긴다. 같은 order 0인 OBJ_*와는 거리로 갈린다(이 Quad는 카메라 자식으로 60유닛 앞이고
OBJ는 10유닛 앞이라, 직교 카메라 뒤→앞 정렬에서 항상 보스가 먼저=뒤에 그려진다). 씬을 안 건드리고
원하는 순서가 나오는 값이라 0으로 확정. 또 이 값은 **인스펙터에 노출하지 않고 const로 박았다** —
public 필드로 뒀더니 씬에 남은 예전 값(-100)이 스크립트 기본값을 이겨서 "보스가 사라지는데 코드엔
원인이 안 보이는" 상황이 실제로 났다.

**4. 보스와 겹치는 모든 요소의 흰 아웃라인** — 새 렌더러 피처 `BossOverlapOutlineFeature`
(+ `Hidden/BossOverlapMask`, `Hidden/BossOverlapOutline`). ① 대상 레이어(Default·Player·Ground·
Wall·Enemy)를 마스크 머티리얼로 한 번 더 그려 요소 실루엣을 만들고 ② 그 안쪽 가장자리를 뽑아
보스 RT 알파가 있는 픽셀에서만 흰색으로 덧그린다. 1번 수정으로 보스 RT가 화면과 1:1이라 좌표
변환 없이 같은 uv로 두 텍스처를 샘플링한다. 보스 Quad 자신은 `renderingLayerMask` 비트1로
빼서(마스크 패스는 비트0만 수집) 화면 테두리에 선이 생기지 않게 했다.

**4번에서 실측으로 잡은 함정 2개** (둘 다 "선이 한 픽셀도 안 나온다"로 똑같이 보였다):
- **마스크를 알파 채널에 담으면 안 된다.** 이 프로젝트는 HDR + 32Bit라 URP 색 버퍼가
  `B10G11R11_UFloatPack32`(알파 없음)다. 마스크 텍스처 desc를 카메라 색에서 복사하니 알파가 없어
  `.a`가 전부 1로 읽혔다. → 마스크 포맷을 `R8G8B8A8_UNorm`으로 못박고, 커버리지 자체는 프리멀티
  플라이드로 **RGB**에 담아 `.r`로 읽는다(포맷에 안 흔들림).
- **아웃라인 대상 레이어에 Default(0)를 넣으면 안 된다.** 배경 하늘 `d`가 레이어 0이라 마스크가
  화면 전체 1이 되어 "가장자리"가 사라진다. 기본값을 Player·Ground·Wall·Enemy(3840)로 한정.
- (부수) `targetTexture != null`이면 패스를 스킵하도록 짰더니 MCP 스크린샷 캡처(카메라→RT 렌더)에서
  패스가 빠져 오진을 유발했다 → 보스 픽셀 카메라만 정확히 제외하도록 수정.

**선 두께·끊김 후속 수정**(사용자 리포트 "더 얇게, 안 깨지게"): 두께 기본값 1.5 → **1픽셀**,
그리고 두께를 **정수 픽셀로 반올림**한다 — 마스크를 포인트 샘플링하므로 1.5픽셀 오프셋이면
프래그먼트의 서브픽셀 위치에 따라 이웃이 1↔2픽셀로 튀면서 선이 점선처럼 끊긴다. 추가로 보스
실루엣 판정을 중심 한 점이 아니라 **RT 1텍셀 이웃까지 max로 부풀려서** 판정한다(보스 RT가
272x153로 저해상도라 가장자리에서 판정이 튀는 것을 흡수).

**검증(플레이 모드 실측, MCP)**: `Renderer2D.asset`에 피처 등록 완료(에디터 API로 sub-asset + 맵
갱신). 컴파일 에러 0(기존 경고 2건만). 48x27 보스룸에서 `camOrtho=13.5 / quadScale=48x27` 일치,
보스가 잘리지 않고 전체가 보임, 하늘·지형·플레이어가 보스 빛을 받음, 광원 오브젝트를 보스 실루엣
위로 옮겨 두니 **보스 앞에 그려지고 흰 아웃라인이 끊김 없이 1픽셀로 둘러진 것**을 확대 스크린샷으로
확인(`Assets/Screenshots/outline_thin_check.png`, `boss_final.png`). 씬은 더티 아님(dirty=False).

⚠️ 확인 중 발견한 별개 이슈: 플레이 진입 시 `Time.timeScale=0.4` + `Grayscale.Intensity=1`이 남아
있었다(이전 세션 상태로 보임). 이번 변경과 무관하지만 화면이 흑백·슬로우로 시작하니 확인 필요.

---

## 2026-08-09 — 보스 레이저 추격 + 노출 붕괴(노이즈 · HP 감소)

**1. 빔이 플레이어를 추격(약간 느리게)** — 기존엔 각속도(180°/s)로만 플레이어를 겨눴다. 그건
"추격"이 아니라 "조준"이라 멀리 있으면 순식간에 따라붙고 붙어 있으면 절대 못 벗어난다. 월드
공간 조준점 `beamAimPoint`를 두고 `Vector3.MoveTowards`로 **플레이어 이동속도 × 0.85**(= 6 × 0.85
= 5.1 u/s)로 뒤쫓게 한 뒤 빔이 그 점을 겨눈다 → 계속 달리면 부채꼴 밖으로 빠져나갈 수 있다.
기존 각속도 클램프는 조준점이 눈 바로 옆을 스칠 때 각도가 튀는 것만 막는 안전장치로 남겨 뒀다.

**2. 3초 이상 노출 → 노이즈 + 5초마다 HP 1칸** (`BossEyeTracker.UpdateExposure`) — 노출 판정은
사거리(`beamRange`)와 외곽각(`beamOuterAngle`, Light2D는 **전체각**이라 반각과 비교) 두 가지뿐이다
(지형 차폐는 안 본다 — 2D 부채꼴 빛 자체가 벽을 무시한다). 3초를 넘기면 자아 고갈과 **같은**
`ScreenGlitchFx`를 켜고, 그 뒤 5초마다 `TakeDamage(1)`. 노출이 한 프레임이라도 끊기면 카운트 ·
노이즈 · 피해 주기가 전부 0으로 초기화된다. 첫 피해는 자아 붕괴 규칙과 동일하게 디버프가 걸린
뒤 한 주기를 채워야 들어온다(노출 3초 + 5초 = 8초째).

**3. ScreenGlitchFx에 원인(Source) 구분 추가** — 노이즈를 켜는 주체가 자아 고갈 · 폭주 심박 ·
보스 빔 셋이 됐다. 예전엔 아무나 `End()`를 부르면 다 꺼져서, 폭주 중(`DrainEgo`가 자아>0이면 매
프레임 `End()`) 보스 빔 노이즈가 즉시 꺼지는 문제가 생긴다. `Begin/End(Source)`로 비트마스크를
쌓고 **남은 원인이 없을 때만** 페이드아웃한다. 기존 호출부는 기본 인자(Ego)라 그대로 동작하고,
심박만 `Source.Heartbeat`로 바꿨다(덤으로 심박 End가 자아 노이즈를 끄던 잠재 버그도 사라짐).

**검증(플레이 모드 실측, MCP)**: Editor.log의 `[EVENT] boss_beam` 기준
`exposure_break after=3.01s` → 첫 피해 `08.64s` → 이후 `13.63 / 18.63 / 23.63 / 28.63 …` 정확히
...
## 2026-08-13 — 부패충 3종(소형·중형·거대 군주) 구현 · 부유 추격 AI

기획안 "전체_몬스터_기획"의 게헨나 몹 3종을 `Assets/Sprites/Bug` 에셋으로 구현했다.
중형은 사용자 확인대로 **외형·애니메이션만 다른 스킨 2종**(`MediumBug` / `MediumBug2`)이라
스탯은 같고 리치·판정 반경만 각자 아트에 맞췄다.

### 1. 스프라이트 재슬라이스 (선행 필수)
기존 자동 슬라이스가 **프레임마다 타이트 박스 + 개별 피벗**이라 재생하면 몸이 덜덜 떨렸고,
Death 시트는 튀는 조각까지 개별 스프라이트로 잘려 프레임 수가 어긋나 있었다
(중형 Death 실제 10프레임 → 18개, 거대 Death 13프레임 → 26개).

파일명의 셀 크기로 균일 격자 분할(피벗 Center, 나머지 임포터 설정은 기존 시트와 동일):

| 개체 | 셀 | IdleMove / Attack / Death |
|---|---|---|
| 소형 | 20×28 | 4 / 6 / 10 |
| 중형 | 34×37 | 8 / 10 / 10 |
| 중형2 | 88×37 | 8 / 8 / 11 |
...
셀 크기 검증은 합본 시트로 교차 확인했다(예: 거대 합본 936×132 = 13열×3행 = 72×44 격자).

### 2. 애니메이션
`Assets/Animations/Bug/<이름>/` 아래 클립 3개 + 컨트롤러 1개씩.
GehennaHound와 같은 구성 — **전이 없이 상태만 나열**하고 코드가 `anim.Play(이름)`으로 직접 튼다.

⚠️ Unity는 PPtr 클립 길이를 이미 `(마지막 키 + 1프레임)`으로 잡는다. 마지막 프레임을 온전히
보이게 하려고 종료 키를 하나 더 찍으면 **클립이 1프레임 길어지고 마지막 스프라이트만 2프레임
동안** 보인다(실측: 4프레임 12fps → 0.417초). 여분 키 없이 그냥 n개만 찍는 게 맞다
(GehennaHound-Run 실측: 키 8개 0~0.5833, `m_StopTime` 0.6666 = 8/12).

**Attack 클립 fps는 아트의 "때리는 프레임" 위치에 맞춰 정했다.** 소형은 6프레임 중 2번째에서
초승달 베기가 나오는데, 12fps(0.5초)로 두면 선딜이 0.09초까지 줄어 반응이 불가능해진다 →
8fps(0.75초)로 늘려 선딜 0.18초를 확보. 거대는 6fps(1.0초)로 늘려 육중한 선딜 0.45초.
결과적으로 **네 개체 모두 `공격 클립 길이 == 논리 공격 길이`, 재생속도 배율 0.99~1.00**이 됐다.

### 3. `CorruptedBug.cs`
GehennaHound와 같은 이유로 `DummyEnemy` 상속(PlayerController의 전투 판정이 그 타입에 고정).
개와 다른 점은 "떠다닌다"는 것뿐 — 수면/기상/배회 단계가 없고 `gravityScale = 0`이다.

- **부유**: 수직 속도를 매 프레임 직접 잡는다. `sin` 위치가 아니라 그 미분(`cos·amp·speed`)을
  **속도로** 넣는다 — Transform을 직접 옮기면 벽·바닥을 통과한다.
- **y 이동 억제**: 목표 높이와의 차이가 `verticalDeadzone` 안이면 y를 아예 안 건드린다.
  "플레이어가 y축 이동을 할 때를 제외하면 y 이동이 거의 없다"는 스펙이 여기서 나온다.
- **수직 리쉬**: `DummyEnemy.leashRange`는 x축만 막아준다 → 목표 y를 `앵커 ± verticalLeash`로
  클램프해 플레이어를 따라 맵 밖까지 떠오르는 걸 막는다.
- **공격 중 완전 정지**: 판정 캡슐이 몸 기준이라 보빙하면 예고 위치와 실제 판정이 어긋난다.
- **수직 정렬 게이트**: `DummyEnemy`의 공격 개시 조건이 `|dx|` 하나뿐이라, 플레이어가 바로
  위/아래에 있으면 허공을 계속 친다. 높이 차가 `attackYTolerance`보다 크면 `base.Update()`를
  아예 안 태우고 수평 추격만 직접 굴린다(`TickTimers()`는 따로 호출 — 안 그러면 쿨다운이 멈춘다).
- **판정 캡슐**: 창(spear) 로컬 좌표 대신 **월드 오프셋**(`attackReachNear/Far`)으로 바꿔 끼웠다.
  프리팹 스케일이 6.5라 로컬 좌표를 쓰면 인스펙터 숫자와 실제 도달거리가 6.5배 어긋나 튜닝이 안 된다.
- **슈퍼아머(거대 군주)**: `DummyEnemy.TakeDamage(int,float)`를 `virtual`로 열고(한 단어, 기본
  동작 변경 0) 오버라이드 — 넉백 거리를 0으로 넘기고 base가 세운 Hitstun을 맞기 직전 상태로
  되돌려 **진행 중이던 공격이 안 끊긴다**. 되돌리지 못하는 건 `lastAttackNeutralized`(초월 예고
  원이 "터지지 않고 사라짐"으로 보이는 연출 플래그) 하나뿐이다.
- **피격 플래시**: 개는 `state`가 Hitstun이 되는 상승 엣지로 켜지만, 여기선 슈퍼아머가 Hitstun을
  즉시 되돌려 엣지를 놓칠 수 있어 `TakeDamage`에서 직접 켠다.

### 4. 스케일 6.5의 근거
플레이어는 PPU 20 × 스케일 1.3 = **0.065 유닛/px**. 벌레는 PPU 100이므로 같은 픽셀 밀도가
되려면 `0.065 × 100 = 6.5`. 결과 크기(플레이어 콜라이더 0.75×1.45 기준): 소형 0.65×0.52,
중형 0.98×0.91, 중형2 1.04×1.43, 거대 1.43×1.69 — 기획의 대소 관계와 맞는다.

### 5. 범위 밖(의도적 미구현)
- 중형 "가드 시 밀려납니다" — **플레이어에 가드 기능 자체가 없다**. 적용 대상 없음.
- 거대 군주 "독 상태이상(지속 피해)" — 상태이상 시스템이 없어 새로 만들어야 하는데,
  **사용자 결정으로 제외**하고 데미지 2칸짜리 단일 강타로 대체했다.
- 중형 "연속 베기" — `DummyEnemy`는 판정 창이 1회뿐이다. 기획서의 "또는 크게 휘두릅니다"를
  택해 긴 리치 단타로 구현.

### 6. Play 실측 (전부 PASS)
| 검증 | 결과 |
|---|---|
| 프리팹 정합(클립 이름·gravity 0·플래시 머티리얼·사망 포즈 ≥ 클립·공격 정렬) | PASS |
| 아이들 중 x 고정 · y는 보빙 범위 안에서만 | dx 0.000, 속도 0.242 ≤ 보빙 최대 0.256 |
| 감지 → 추격 | `bug_state: aggro_start`, 소형 −78 → −69.9 이동 |
| 플레이어 y 이동 추적 | 플레이어 +6 즉시 상승속도 2.88(상한 3.2), x는 불변 |
| 목표 높이 도달 후 정지 | 거대 24.80 → 26.37(앵커 26.60, 데드존 0.35 안) |
| 공격이 실제로 피해를 줌 | `hit_player dmg=1` 8회, 플레이어 HP 8 → 0 |
| 거대 슈퍼아머 / 일반 개체 경직 | Chase 유지·HP 14→13 / Hitstun 진입·HP 3→2 |
| 사망: Death 클립 → 포즈 유지 → 비활성화 | PASS |

⚠️ **Map-test에서 플레이 검증할 때의 함정**: 인트로 연출이 `DummyEnemy.AiFrozen = true`를
걸어두고 플레이어를 자동으로 움직인다. AI가 아예 안 도는 것처럼 보이므로 `AiFrozen = false`를
먼저 풀어야 한다. 또 플레이어를 텔레포트로 옮기면 낙사 판정이 나 사망 연출이 `Time.timeScale`을
0/0.04로 잡아 **게임 시간이 멈춘 채 프레임만 도는** 상태가 된다(측정값이 전부 정지해 보인다).
플레이어를 건드리는 대신 **적을 플레이어 옆으로 옮기거나**, 물리만 볼 때는 플레이어를
`SetActive(false)`로 잠시 꺼두는 게 안전하다.

### 7. 배치
Map-test 플레이어 스폰(−85.9, 24.2) 오른쪽 평지에 x = −78 / −70 / −62 / −54,
y는 지면(24.0)보다 2~2.6 위에 띄워 배치. 몸이 큰 개체일수록 `hoverYOffset`을 키워
(0.5 / 0.6 / 0.9 / 1.0) 플레이어에게 내려올 때 바닥에 파묻히지 않게 했다.

### 부패충 flipX 위치 튐 수정 + 전 시트 셀 분할 (2026-08-13 후속)

사용자 리포트: "얘네 flipx하면 위치가 바뀌는 문제 있습니다."

**원인**: 피벗이 **셀 중앙**에 있는데 몸통은 셀 안에서 왼쪽으로 치우쳐 있었다. 좌우 반전은
`localScale.x` 부호를 뒤집는 방식이라 몸이 피벗 반대편으로 미러링돼 `(셀중앙 − 몸중심) × 2`
만큼 통째로 튄다. 실측 이동량(스케일 6.5 기준):

| | 몸중심 px | 셀중앙 px | flip 시 튐 |
|---|---|---|---|
| 소형 | 8.0 | 10 | 0.26 유닛 |
| 중형 | 15.0 | 17 | 0.26 유닛 |
| 중형2 | 31.5 | 44 | **1.63 유닛** |
...
중형2가 유독 심한 건 셀 폭 88px 중 44px을 오른쪽 분사 이펙트용 여백으로 쓰기 때문이다.

**수정**: 피벗을 셀 중앙이 아니라 **몸통 중심**으로 옮겼다(alignment=Custom).
몸통 중심은 IdleMove 시트의 **알파 바운딩 박스 합집합**을 픽셀에서 직접 읽어 구했다
(임포트된 텍스처는 `isReadable=false`라 `File.ReadAllBytes` → `Texture2D.LoadImage`로 새로 디코드).
Attack·Death 시트도 0번 프레임 몸 x범위가 IdleMove와 완전히 같아서 **한 개체당 피벗 하나**를
세 시트에 공통으로 적용했다 — 그래야 클립이 바뀔 때 몸이 안 튄다.
...
콜라이더도 오프셋을 0으로 되돌렸다(피벗 = 몸통 중심이므로 오프셋이 필요 없고, 오프셋이 남아
있으면 그것도 스케일 부호를 따라 반대편으로 미러링돼 같은 문제를 일으킨다).

⚠️ **`SpriteRenderer.bounds`로 flip 안정성을 판단하면 안 된다.** 그건 투명 여백까지 포함한
**셀 전체** 기준이라, 피벗을 고쳐도 수정 전과 똑같은 이동량이 찍힌다(실제로 그렇게 나와 한 번
헛짚었다). 눈에 보이는 건 픽셀이 있는 부분이므로 **알파 바운딩 박스 중심 ↔ 피벗 거리**로 재야 한다.

**실측**: 네 개체 모두 모든 idle 프레임에서 flip 이동량 **0.0000 유닛**.
Play 중 콜라이더 중심 ↔ transform 거리도 좌·우 양방향 모두 **0.0000**.

**공격 판정 재측정**: 피벗이 바뀌었으므로 판정 캡슐도 타격 프레임 픽셀에서 다시 쟀다.
타격 프레임은 "이펙트 폭이 처음 급증하는" 프레임(소형2 · 중형4 · 중형2\_4 · 거대3)이고,
그 뒤 폭이 더 늘어나는 건 흩어지는 물방울이라 판정에서 뺐다(열별 세로 픽셀 수 3 이상까지만 인정).

| | reachFar | heightOffset | hitRadius | attackRange |
|---|---|---|---|---|
| 소형 | 0.78 | −0.46 | 0.29 | 0.66 |
| 중형 | 1.04 | −0.52 | 0.45 | 0.88 |
| 중형2 | 2.96 | −0.26 | 0.39 | 2.50 |
| 거대 | 2.31 | −0.33 | 0.50 | 1.95 |

**전 시트 셀 분할**(사용자 지시): 개별 애니메이션 시트 12장 + 합본 7장 = 19장을 전부 격자로 다시
잘랐다. `.aseprite`(원본 소스)와 `Medium2 bug.png`(136×22, 격자에 안 맞는 별개 이미지)는 제외.
이름이 그대로라 fileID가 유지돼 기존 애니메이션 클립 참조는 전부 살아있다(12개 클립 null 0건 확인).

**"with shadow" 미사용 확인**: GUID 역추적으로 클립·컨트롤러·프리팹·Map-test 씬 25개 파일을 훑어
참조 0건 확인. 실제로 쓰는 건 개별 애니메이션 시트 12장뿐이다.

⚠️ **중형2(`Medium2 bug-*`)만 시트에 갈색 바닥 그림자가 그려져 있다.** 소형·중형·거대는 깨끗하다
(이름에 "with shadow"가 붙은 합본에만 그림자가 있고 그건 안 쓴다). 중형2는 폴더에 그림자 없는
변형이 아예 없어서 교체할 대상이 없다 — 떠다니는 몬스터라 그림자가 몸을 따라 위아래로 같이
움직인다. 색으로 마스킹해 그림자만 지운 사본을 만드는 건 가능하지만 원본 아트 수정이라 보류했다.

### 사망 연출이 안 보이던 버그 + 중형2 바닥 그림자 제거 (2026-08-13 후속 2)

사용자 리포트: "지금 죽는 연출이 잘 안나오는거 같아요".

**원인 (실측 재현)**: `CorruptedBug.Update()`가 `if (!IsAlive) return;`로 먼저 빠져나가는 바람에
`UpdateHitFlash()`가 사망 후 다시는 안 돌았다. 그래서 **죽는 순간 흰색 피격 플래시가 켜져 있었으면
`_FlashAmount`가 1로 박제**돼 사망 연출(터지는 초록 파편 · 떠오르는 해골)이 통째로 새하얀 실루엣이
된다. 연타 콤보의 마지막 타격으로 죽이면 거의 매번 걸린다 — `TakeDamage` 오버라이드가 죽이는
타격에도 `hitFlashTimer = flashDuration`을 세우기 때문.

한 호출 안에서 결정적으로 재현했다(피격 → `UpdateHitFlash` 1회 수동 호출로 흰색 ON → 즉사):
`즉사 직후 _FlashAmount=1`, 이후 아무리 프레임이 흘러도 1 그대로.

⚠️ MCP 라운드트립 사이에 게임 시간이 **수십 초씩** 흐른다(실측: t 16.69 → 57.98). 프레임 단위
타이밍 버그는 호출을 나눠서 재현할 수 없다 — private 메서드를 리플렉션으로 직접 굴려 한 호출
안에서 시퀀스를 만들어야 한다.

**수정**: `UpdateHitFlash()`를 모든 조기 return보다 앞으로 옮겼다. 죽은 뒤에도 플래시 타이머가
계속 내려가 정상적으로 꺼진다(실측: 사망 후 12프레임 뒤 `_FlashAmount=0`).

**추가 폴리싱**: Death 클립이 12fps라 "터짐 → 파편 → 해골 상승"이 순식간에 지나갔다.
0번 프레임 다음에 바로 터지는 구성이라 더 그렇다. 9fps(거대는 8fps)로 늦추고
`deadPoseDuration`도 클립 길이 + 0.15초로 다시 맞췄다.

| | 프레임 | 이전 | 이후 | deadPose |
|---|---|---|---|---|
| 소형 | 10 | 12fps 0.833초 | 9fps 1.111초 | 1.26 |
| 중형 | 10 | 12fps 0.833초 | 9fps 1.111초 | 1.26 |
| 중형2 | 11 | 12fps 0.917초 | 9fps 1.222초 | 1.37 |
| 거대 | 13 | 10fps 1.300초 | 8fps 1.625초 | 1.78 |

**중형2 바닥 그림자 제거**(사용자 승인): 떠다니는 몬스터인데 시트에 갈색 바닥 그림자가 그려져 있어
몸을 따라 위아래로 같이 움직였다. 팔레트가 10색뿐이라 그림자 색을 정확히 특정할 수 있었다 —
**#443134 rgb(68,49,52), 1340px, 100%가 셀 하단 10줄에만 존재**. 이 색만 알파 0으로 만들었다
(IdleMove 342px · Attack 293px · Death 35px). 실제로 쓰는 3장만 건드리고 합본
`Medium2 bug 88x37.png`와 `.aseprite` 원본은 그림자가 남은 채로 뒀다(참조용).
원본은 git 커밋 e1fb07d에 있어 `git checkout --`로 되돌릴 수 있다.

⚠️ 그림자를 지우면 **몸통 알파 합집합이 바뀌어 피벗도 다시 잡아야 한다**. 중형2 합집합이
y2-27 → y6-27로 줄어 피벗 y가 0.4054 → **0.4595**로 이동. 그에 맞춰 재슬라이스하고 콜라이더
(0.136×0.176, 오프셋 0)와 공격 높이(-0.26 → **-0.39**)도 다시 계산했다.
flip 안정성은 재측정에서도 0.0000 유지.

### E홀드 줌 오작동 수정 · 거대 군주 공격속도 · 부패충 판정 재배선 (2026-08-13 후속 3)

#### 1. "카메라가 작을 때 E홀드하면 이상한 곳에 줌인" (SectionCamera)

**원인**: `SetSustainedFocus`(E홀드 광원 소모 줌)가 팬·줌을 **룸 기준값**(`basePos`·`baseOrthoSize`)으로
계산하는데, 카메라는 **컷신 프레이밍 값**(`framedBasePos`·`framedOrthoSize`)으로 그려진다.
Map-test는 시작부터 `cutsceneBlend=1`이라 둘이 크게 벌어져 있다 — Play 실측:

| | 계산 기준(base) | 실제 적용(framed) |
|---|---|---|
| 화면 중심 | (−87.00, 36.80) | (−77.99, 27.95) |
| ortho | 13.50 | **5.00** |

그래서 줌 감소량이 `-(13.5 − 13.5/1.3) = −3.115`로 잡혀 **ortho 5 → 1.885 (1.3배가 아니라 2.65배)**,
팬 오프셋도 엉뚱한 기준으로 더해져 카메라가 플레이어에서 (9.0, −8.9)만큼 빗나간 곳에 꽂혔다.

**수정**: `SustainedFocusRampCo`가 매 프레임 `framedBasePos`·`FramedOrthoSizeOrBase`를 기준으로
계산하도록 바꿨다. `cutsceneBlend`가 0이면 framed\* == base\* 라 평상시 동작은 완전히 그대로다(회귀 0).
쓰이지 않던 지역변수 `targetOrthoSize`도 같이 정리했다.

**Play 실측**(플레이어 고정 후): 카메라 위치가 플레이어와 **(0.000, 0.000)**, ortho **3.846 = 5 ÷ 1.3**.

#### 2. 거대 군주 벌레 공격 속도

기존 6fps 1.0초 클립 + 선딜 0.45 + 쿨다운 1.5 = 한 사이클 **2.50초**였다.
8fps 0.75초 + 쿨다운 1.0으로 낮춰 **1.75초**(중형 1.73초와 비슷한 박자)로 맞췄다.

#### 3. 공격 판정을 사용자 배치물에 연결 (사용자 지시)

**(a) 애니메이션 이벤트 = 피해 확정 시각.** 사용자가 Attack 클립의 특정 프레임에 이벤트를 찍어뒀다
(함수명은 비어 있었다). 이벤트가 직접 피해를 주게 만들면 DummyEnemy의 판정 규칙(패링·회피·무적·
넉백)을 통째로 우회하게 되므로, 반대로 **파이프라인의 피해 확정 시각이 이벤트 프레임과 겹치도록
선딜·후딜을 Awake에서 역산**한다(`AlignAttackTimingToEvent`).
`windup = eventTime − thrust×thrustHitNormalized − dodgeWindowPost`, `recover = 클립길이 − windup − thrust`.
이벤트를 옮기면 다음 Play에서 자동으로 다시 맞으므로 인스펙터를 손댈 필요가 없다.
함수명은 `AttackHitFrame`(진단 로그 전용)으로 채웠다.

| | 이벤트 | 선딜 | 후딜 | 피해확정 vs 이벤트 |
|---|---|---|---|---|
| 소형 | 0.250 | 0.115 | 0.535 | 오차 0.0000 |
| 중형 | 0.333 | 0.198 | 0.535 | 오차 0.0000 |
| 중형2 | 0.333 | 0.198 | 0.368 | 오차 0.0000 |
| 거대 | 0.375 | 0.240 | 0.410 | 오차 0.0000 |

Play 로그 실측: 이벤트 프레임과 `hit_player`가 **같은 프레임 또는 1프레임 이내**(f1363/f1363,
f1741/f1741, f2026/f2027 …). 재생속도 배율도 네 개체 전부 1.000.

**(b) R/L 상자 = 공격 범위.** 수치 필드(`attackReachNear/Far`·`attackHeightOffset`)는 제거하고
씬에 배치한 `R`/`L` 자식이 유일한 기준이 됐다. `hitRadius`(상자 높이의 절반)와 `attackRange`
(상자 바깥 끝까지의 거리)도 Awake에서 상자에서 유도한다.

⚠️ **`TransformPoint`·`Collider2D.bounds`를 쓰면 안 된다.** 좌우 반전이 부모 `localScale.x` 부호라
`TransformPoint`를 쓰면 왼쪽을 볼 때 L 상자까지 미러링돼 오른쪽으로 넘어간다(R/L을 따로 그린 의미가
사라짐). 부모 스케일의 **절대값만** 곱해 사용자가 그린 위치를 그대로 쓴다. `bounds`는 이 상자들이
**비활성**이라 물리가 갱신해 주지 않아 못 쓴다.

⚠️ 상자 중심은 **월드 좌표**다 — `attackRange`를 낼 때 몸 위치를 빼야 한다.
안 빼서 attackRange가 78.25(씬 좌표 크기)로 나왔었다.

Play 실측 캡슐(몸 기준): 소형 R −0.25~0.78 / L −0.78~0.26, 중형 R −0.19~1.04 / L −1.04~0.19,
중형2 R 0.76~3.42 / L −3.42~−0.76, 거대 R 1.05~2.66 / L −2.63~−1.02 — 사용자가 그린 상자와 정확히 일치
(거대의 비대칭까지 그대로).

#### 4. 플레이어 ↔ 부패충 물리 충돌 없음

레이어 충돌 매트릭스를 전역으로 끄면 같은 Enemy 레이어인 게헨나 포식견까지 영향을 받으므로,
벌레 프리팹의 `Rigidbody2D.excludeLayers`에 Player + PlayerInvincible만 넣었다.
`excludeLayers`는 물리 충돌만 막고 `OverlapCircle` 등 **쿼리는 영향을 안 받으므로** 벌레의 공격
판정도 플레이어의 검 판정도 그대로 동작한다.
Play 실측: 콜라이더가 겹친 상태에서 양쪽 x 이동 **0.000**.

#### 5. 사거리 안이면 이동 정지

수평은 `DummyEnemy.ChaseLogic`이 이미 0으로 누르고 있었고, **수직 추적**이 계속 돌아 사거리 안에서도
플레이어 높이를 쫓아 올라갔다. `IsInAttackRange()`일 때 수직 추적만 끄고 보빙은 남겼다(떠 있는 느낌 유지).

#### 남은 것
R/L 상자와 몸 콜라이더 수정분은 **씬 인스턴스 오버라이드**로만 존재한다(프리팹 원본엔 자식 0개).
지금 씬에서는 완전히 동작하지만, 프리팹을 새로 끌어다 놓으면 상자가 없어 경고 로그와 함께
DummyEnemy 기본 판정으로 폴백한다. 프리팹에 Apply할지는 사용자 확인 후 진행.

### 프리팹 Apply — R/L 상자·몸 콜라이더를 인스턴스 오버라이드에서 프리팹으로 (2026-08-13 후속 4)

사용자가 씬 인스턴스에 직접 만든 R/L 판정 상자와 수정한 몸 콜라이더를 프리팹 원본에 반영했다.
이걸 안 하면 프리팹을 새로 끌어다 놓은 개체는 상자가 없어 기본 판정으로 폴백한다.

⚠️ **오버라이드를 통째로 Apply하면 안 된다.** 인스턴스의 위치(−78, 26 등)까지 프리팹에 박혀
새로 뽑는 개체가 전부 그 좌표로 튀어나온다. `ApplyAddedGameObject`(R·L)와
`ApplyObjectOverride`(BoxCollider2D·SpriteRenderer)만 선별해 호출했다 — 루트 Transform의
위치는 Unity가 *default override*로 분류해 `GetObjectOverrides`에 애초에 안 들어오므로
이 방식이면 자동으로 제외된다(실측 확인: Apply 후 프리팹 원본 위치 (0,0,0) 유지).

SpriteRenderer 오버라이드는 `m_FlipX` 하나였는데 인스턴스·프리팹 둘 다 `False`인 무의미한
잔여값이라(사용자가 flipX를 켰다 껐던 흔적) 같이 정리했다 — 동작 변경 0.

**실측**:
- 프리팹 4종 전부 자식 2개(R/L), R 바깥끝 0.78 / 1.04 / 3.42 / 2.66 — 사용자가 그린 값 그대로
- 프리팹 몸 콜라이더 = 인스턴스와 일치, 프리팹 원본 위치 (0,0,0) 유지, 인스턴스 위치도 그대로
- 남은 오버라이드 0개, 씬 저장 완료
- **프리팹에서 새로 Instantiate한 LordBug도 attackRange 2.66 · hitRadius 0.22로 자립** (Apply의 목적 달성)
